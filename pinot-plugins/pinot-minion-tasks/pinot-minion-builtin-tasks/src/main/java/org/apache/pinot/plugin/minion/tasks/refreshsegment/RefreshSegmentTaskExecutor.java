...
public class RefreshSegmentTaskExecutor extends BaseSingleSegmentConversionExecutor {
   ...
   * Uses the new ColumnReaderFactory-based approach for true columnar segment building.
   */
  private void refreshSegmentColumnWise(File indexDir, File workingDir, TableConfig tableConfig,
    ...
    try {
      ...
      // Create column reader factory for the source segment
      PinotSegmentColumnReaderFactory columnReaderFactory = new PinotSegmentColumnReaderFactory(sourceSegment);
      columnReaderFactory.init(schema);

      // Initialize driver with columnar data source
      driver.init(config, columnReaderFactory);

      // Build the segment using column-wise approach
      driver.build();
